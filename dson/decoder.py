...
def DSONObject(s_and_end, encoding, strict, scan_once, object_hook,
    ...
    while True:
        ...
        if s[end-3:end] == 'wow':
            break
        elif s[end-3:end] != 'and':
            raise ValueError(errmsg("Expecting 'and' delimiter", s, end - 3))
...
def DSONArray(s_and_end, scan_once, _w=WHITESPACE.match, _ws=WHITESPACE_STR):
    s, end = s_and_end
    values = []
    nextchar = s[end:end + 1]
    if nextchar in _ws:
    ...
    while True:
        ...
        if s[end-3:end+1] == 'many':
            end += 1
            break
        elif s[end-3:end] != 'and':
            raise ValueError(errmsg("Expecting 'and' delimiter", s, end))
        try:
            if s[end] in _ws:
                end += 1
                if s[end] in _ws:
...
class DSONDecoder(object):
    """Simple DSON <http://json.org> decoder
    ...
    their corresponding ``float`` values, which is outside the DSON spec.
    ...
    def __init__(self, encoding=None, object_hook=None, parse_float=None,
        ...
        of every DSON object decoded and its return value will be used in
        place of the given ``dict``.  This can be used to provide custom
        deserializations (e.g. to support DSON-RPC class hinting).

        ``object_pairs_hook``, if specified will be called with the result of
        every DSON object decoded with an ordered list of pairs.  The return
        ...
        self.strict = strict
        self.parse_object = DSONObject
        self.parse_array = DSONArray
