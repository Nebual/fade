...
pretty funny data-interchange format.

This implementation was adapted from the official Python2.7 json library by Nebual.

Encoding basic Python object hierarchies::
    >>> import dson
    >>> dson.dumps(['foo', {'bar': ('baz', None, 1.0, 2)}])
    'so "foo" and such "bar" is so "baz" and gone and 1.0 and 2 many wow many'		Awesome DSON version
    '["foo", {"bar": ["baz", null, 1.0, 2]}]' 										Boring JSON version
    >>> print dson.dumps({"c": 0, "b": 0, "a": 0}, sort_keys=True)
    such "a" is 0 and "b" is 0 and "c" is 0 wow		Awesome DSON version
    {"a": 0, "b": 0, "c": 0}						Boring JSON version
...
__author__ = 'Bob Ippolito <bob@redivi.com>' #of the json library

from .decoder import DSONDecoder
from .encoder import DSONEncoder

_default_encoder = DSONEncoder(
    skipkeys=False,
    ensure_ascii=True,
    check_circular=True,
    allow_nan=True,
...
def dumps(obj, skipkeys=False, ensure_ascii=True, check_circular=True,
    ...
    strict compliance of the DSON specification, instead of using the
    JavaScript equivalents (``NaN``, ``Infinity``, ``-Infinity``).
...
_default_decoder = DSONDecoder(encoding=None, object_hook=None,
